...
public class Chip : AppCompatTextView {
    ...
    private fun initTypedArray(attrs: AttributeSet?) {
        ...
        chipBackgroundColor = ta.getColor(
            R.styleable.Chip_mcv_backgroundColor,
            ContextCompat.getColor(context, R.color.colorChipBackground)
        )
        chipSelectedBackgroundColor = ta.getColor(
            R.styleable.Chip_mcv_selectedBackgroundColor,
            ContextCompat.getColor(context, R.color.colorChipBackgroundClicked)
        )
        chipTextColor = ta.getColor(
            R.styleable.Chip_mcv_textColor,
            ContextCompat.getColor(context, R.color.colorChipText)
        )
        chipSelectedTextColor = ta.getColor(
            R.styleable.Chip_mcv_selectedTextColor,
            ContextCompat.getColor(context, R.color.colorChipTextClicked)
        )
        chipCloseColor = ta.getColor(
            R.styleable.Chip_mcv_closeColor,
            ContextCompat.getColor(context, R.color.colorChipCloseInactive)
        )
        chipSelectedCloseColor = ta.getColor(
            R.styleable.Chip_mcv_selectedCloseColor,
            ContextCompat.getColor(context, R.color.colorChipCloseClicked)
        )
        cornerRadius = ta.getDimensionPixelSize(
            R.styleable.Chip_mcv_cornerRadius,
            resources.getDimensionPixelSize(R.dimen.chip_height) / 2
        )
        strokeSize = ta.getDimensionPixelSize(R.styleable.Chip_mcv_strokeSize, 0)
        strokeColor = ta.getColor(
            R.styleable.Chip_mcv_strokeColor,
            ContextCompat.getColor(context, R.color.colorChipCloseClicked)
        )
        val iconText = ta.getString(R.styleable.Chip_mcv_iconText)
        val iconTextColor = ta.getColor(
            R.styleable.Chip_mcv_iconTextColor,
            ContextCompat.getColor(context, R.color.colorChipCloseClicked)
        )
        val iconTextBackgroundColor = ta.getColor(
            R.styleable.Chip_mcv_iconTextBackgroundColor,
            ContextCompat.getColor(context, R.color.colorChipBackgroundClicked)
        )
    ...
    private fun createPaddings() {
        ...
        val startPadding =
            if (chipIcon == null && chipIconBitmap == null && iconText == null) resources.getDimensionPixelSize(
                R.dimen.chip_icon_horizontal_margin
            ) else 0
        val endPadding =
            if (selectable || closable) resources.getDimensionPixelSize(R.dimen.chip_close_horizontal_margin) else resources.getDimensionPixelSize(
                R.dimen.chip_icon_horizontal_margin
            )
        setPaddingRelative(startPadding, 0, endPadding, 0)
        compoundDrawablePadding =
            resources.getDimensionPixelSize(R.dimen.chip_icon_horizontal_margin)
    }
    ...
    private fun createChipIcons() {
        ...
        if (chipIcon == null && chipIconBitmap == null && iconText == null) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
                setCompoundDrawablesRelativeWithIntrinsicBounds(
                    null,
                    null,
                    if (selectable) selectIcon else closeIcon,
                    null
                )
            } else {
        ...
        if (chipIconBitmap != null) {
            ...
            chipIconBitmap =
                ChipUtils.getCircleBitmap(context, chipIconBitmap, cornerRadius.toFloat())
        ...
        if (iconText != null && iconText != "") {
            val textBitmap = ChipUtils.getCircleBitmapWithText(
                context,
                iconText!!, iconTextColor, iconTextBackgroundColor, cornerRadius.toFloat()
            )
        ...
        return ContextCompat.getDrawable(context, R.drawable.ic_close)?.apply {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                mutate().colorFilter = BlendModeColorFilter(
                    if (chipSelected) chipSelectedCloseColor else chipCloseColor,
                    BlendMode.SRC_IN
                )
            } else {
    ...
    fun setIconText(text: String, iconTextColor: Int = 0, iconTextBackgroundColor: Int = 0) {
        ...
        this.iconTextColor =
            if (iconTextColor == 0) ContextCompat.getColor(
                context,
                R.color.colorChipBackgroundClicked
            )
            else iconTextColor
        this.iconTextBackgroundColor =
            if (iconTextBackgroundColor == 0) ContextCompat.getColor(
                context,
                R.color.colorChipBackgroundClicked
            )
